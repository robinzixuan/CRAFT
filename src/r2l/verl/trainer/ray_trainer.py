    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # compute latent features (h_mean) using actor model so reward
                # function can skip loading a separate model copy
                with timer("latent", timing_raw):
                    latent_output = self.actor_rollout_ref_wg.compute_latent_features(batch)
                    batch = batch.union(latent_output)
