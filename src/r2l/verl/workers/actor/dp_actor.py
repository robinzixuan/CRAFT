...
class DataParallelPPOActor(BasePPOActor):
    ...
    @torch.no_grad()
    def compute_latent_features(self, data: DataProto) -> torch.Tensor:
        """Compute mean-pooled last hidden state from the actor model.

        This reuses the already-loaded actor model (FSDP) so the reward function
        does not need to load a separate model copy.

        Returns:
            torch.Tensor: h_mean of shape [batch_size, hidden_dim]
        """
        self.actor_module.eval()
        ...
        h_mean_lst = []
        if self.rank == 0:
            micro_batches = tqdm(micro_batches, desc="Compute latent features", position=1)

        # Locate the last transformer decoder layer once, outside the loop.
        # Using a forward hook avoids output_hidden_states=True which stores
        # all 28 layer outputs (~5 GB) — we only need the last one (~200 MB).
        base = self.actor_module.module if hasattr(self.actor_module, "module") else self.actor_module
        last_layer = None
        if hasattr(base, "model") and hasattr(base.model, "layers"):
            last_layer = base.model.layers[-1]  # works for Qwen3, Llama, Mistral, etc.
        ...
        for micro_batch in micro_batches:
            ...
            if "multi_modal_inputs" in micro_batch.non_tensor_batch:
                multi_modal_inputs = batch_collate(micro_batch.non_tensor_batch["multi_modal_inputs"])
                multi_modal_inputs = {key: torch.cat(value, dim=0) for key, value in multi_modal_inputs.items()}
            ...
            if last_layer is not None:
                # Hook captures only the last layer output; no intermediate states stored.
                captured = []
                hook = last_layer.register_forward_hook(
                    lambda m, inp, out: captured.append(
                        (out[0] if isinstance(out, tuple) else out).float().detach()
                    )
                )
                self.actor_module(
                    input_ids=input_ids,
                ...
                hook.remove()
                h_last = captured[0]  # [B, seq_len, hidden_dim]
            else:
                # Fallback for architectures without .model.layers
                output = self.actor_module(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                ...
                h_last = output.hidden_states[-1].float()

            mask = attention_mask.float().unsqueeze(-1)  # [B, seq_len, 1]
            h_mean = (h_last * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1e-6)
            h_mean_lst.append(h_mean)

        h_mean = torch.cat(h_mean_lst, dim=0)

        if self.config.dynamic_batching:
            h_mean = restore_dynamic_batch(h_mean, batch_idx_list)
