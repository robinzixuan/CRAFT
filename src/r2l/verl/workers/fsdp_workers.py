...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        self.model_config.attention_implementation = "sdpa"
        ...
        if (not fsdp_config.enable_rank0_init) or self.device_mesh.get_local_rank("fsdp") == 0:
            model = AutoClass.from_pretrained(
                ...
                attn_implementation="sdpa",  # Changed from flash_attention_2 (not installed)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_latent_features(self, data: DataProto):
        """Compute mean-pooled hidden states using the actor model (FSDP).

        Returns a DataProto with h_mean tensor so the reward function can
        use it directly instead of loading a separate model.
        """
        assert self._has_actor
        ...
        with self.ulysses_sharding_manager:
            ...
            h_mean = self.actor.compute_latent_features(data=data)
            output = DataProto.from_dict(tensors={"h_mean": h_mean})
